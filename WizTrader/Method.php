...
class cMethod{
	...
	*设定基本参数                                                              *
	\**************************************************************************/
	final 
	function Set( /*string*/ $type/*运行的类型*/ , /*int*/ $ymd/*运行的日期YYYYMMDD*/ , /*string*/ $market ='cn.sh.a' /*中国A股*/){
		$this->type = $type;
		$this->ymd = $ymd;
		$this->stock = $this->stock_object[$market];
		$this->math = new cMath();
	...
	* 需要什么样的指标                                                         *
	\**************************************************************************/
	final 
	function Require( /*string*/ $algo /*算法比如MA*/, /*string*/ $field/*对哪个字段*/, /*string*/ $period/*指标周期*/){
	...
	//! 
	final 
	function EntrustBefore(){
		return $this->hook_entrust_before();
	}
	//! 
	final 
	function EntrustAfter(){
	...
	//! 
	final 
	function Price(/*string*/ $stock,/*array*/ $price){
		//! 实时计算数据
		$this->price = price ;
		foreach($this->need as $algo => $param){
			//!    完整的名字             算法         字段            周期
			//!    EMA|OPEN|5             EMA          OPEN            5
			//!    如示调用计算五日开盘均价
			//!    CROSS|OPEN,CLOSE|0       CROSS        OPEN,CLOSE    0
			$field = $param['field'];
			$value = $this->expr( $param['field'] );
 			$price[ $param['name'] ]=$this->$algo($field,$param['period'],$value);
		}
	...
	function Pool(){
		...
		if( in_array("all",$pool) ){
			//! 考虑到市场的多样性，所以返回当前对象允许的清单
			$self->pool = $this->stock('all');
		}else{
	...
	final 
	function H(){
		return $self->stock('high',$this->code);
	}
	final 
	function L(){
		#3， LOW 最低价
		#返回该周期最低价。
		return $self->stock('low',$this->code);
	}
	final 
	function C(){
		#6， C 收盘价
		#返回该周期收盘价。
		#用法： C
		return $self->stock('close',price['close']);
	}		
	final 
	function V(){
		#8， V 成交量
	...
	*/
	...
	function TOPRANGE(){
		...
		return $this->stock('toprange',$A,$B,$N);
	}
	final 
	function LOWRANGE(){
		...
		return $this->stock('lowrange',$A,$B,$N);
	}
	final 
	function CROSS($A,$B,$N){
		...
		return $this->stock('cross',$A,$B,$N);
	}
	final 
	function LONGCROSS($aR1,$aR2,$aPeriod){
		#2， LONGCROSS 维持一定周期后上穿
		#两条线维持一定周期后交叉。
		#用法：　LONGCROSS(A，B，N)　表示A在N周期内都小于B，本周期从下方向上穿过B时返回1，否则返回0。
		return $this->stock('longcross',$aWhich,$aPeriod);
	}
	final 
	function UPNDAY($aWhich,$aPeriod){
		#3， UPNDAY 连涨
		#返回是否连涨周期数。
		#用法：　UPNDAY(CLOSE,M)　表示连涨M个周期。
		return $this->stock('upday',$aWhich,$aPeriod);
	}
	final 
	function DOWNDAY($aPeriod,$aWhich=""){
		#4， DOWNNDAY 连跌
		#返回是否连跌周期。
		#用法：　DOWNNDAY(CLOSE，M)　表示连跌M个周期。
		if( $aWhich == "" ){
			$aWhich = $this->stock_code;
		}
		return $this->stock('downday',$aWhich,$aPeriod);
	}
	final 
	function NDAY($aexp,$an){
		#5， NDAY 连大
		#返回是否持续存在X>Y。
		#用法：　NDAY(CLOSE，OPEN，3)　表示连续3日收阳线。
		return $this->stock('nday',$aexp,$an);
	}
	final 
	function EXIST($aexp,$an){
		#6， EXIST 存在
		#是否存在。
		#用法：　EXIST(CLOSE>OPEN，10)　表示前10日内存在着阳线。
		return $this->stock('exist',$aexp,$an);
	}
	final 
	function EVERY($aexp,$an){	
		#7， EVERY 一直存在
		#一直存在。
		#用法：　EVERY(CLOSE>OPEN，10)　表示前10日内一直阳线。
		return $this->stock('every',$aexp,$an);
	}
	final 
	function LAST($aexp,$an,$am){
		#8， LAST 持续存在
		#用法：　LAST(X,A,B)　 A>B，表示从前A日到前B日一直满足X条件。若A为0，表示从第一天开始，B为0，表示到最后日止。
		#例如：　LAST(CLOSE>OPEN，10，5)　表示从前10日到前5日内一直阳线。
		return $this->stock('last',$aexp,$an,$am);
	}
	final 
	function TESTSKIP(){
	...
	final 
	function opIF($aCond,$aTrue,$aFalse){
		#2， IF 逻辑判断
		#根据条件求不同的值。
		#用法：　IF(X，A，B)　若X不为0则返回A，否则返回B。
		#例如：　IF(CLOSE>OPEN，HIGH，LOW)表示该周期收阳则返回最高值，否则返回最低值。
		return ( $aCond != 0 ) ? $aTrue : $aFalse ;
	}
	final 
	function opIFN(){
		#4， IFN 逻辑判断
	...
	final 
	function AVEDEV($aseq,$an){
		#统计函数
		#1， AVEDEV 平均绝对方差
		#AVEDEV(X，N) 　返回平均绝对方差。
		return $this->math('avedev',$aseq,$an);
	}
	final 
	function DEVSQ($aseq,$an){
		#2， DEVSQ 数据偏差平方和
		#DEVSQ(X，N) 　返回数据偏差平方和。
		return $this->math('devsq',$aseq,$an);
	}
	final 
	function FORCAST($aseq,$an){
		#3， FORCAST 线性回归预测值
		#FORCAST(X，N)　 返回线性回归预测值。
		return $this->meth('forcast',$aseq,$an);
	}
	final 
	function SLOPE($aseq,$an){
		#4， SLOPE 线性回归斜率
		#SLOPE(X，N)　 返回线性回归斜率。
		return $this->math('slope',$aseq,$an);
	}
	final 
	function STD($aseq,$an){
		#5， STD 估算标准差
		#STD(X，N)　 返回估算标准差。
		return $this->math('std',$aseq,$an);
	}
	final 
	function STDP($aseq,$an){
		#6， STDP 总体标准差
		#STDP(X，N)　 返回总体标准差。
		return $this->math('stdp',$aseq,$an);
	}
	final 
	function VAR($aseq,$an){
		#7， VAR 估算样本方差
		#VAR(X，N)　 返回估算样本方差。
	}
	final 
	function VARP($aseq,$an){
		#8， VARP 总体样本方差
		#VARP(X，N)　 返回总体样本方差 。
	}
	final 
	function	BLOCKSETNUM($aname){
	...
	function MYSORTIDX(){
		...
		return $this->stock($this->today,$this->stock,'sort_index');
	}
	final 
	function ALLSTKNUM($aMarket="sh"){
		#16， ALLSTKNUM 交易股票数
		#返回当日所有交易股票数,该函数需要扩展数据支持
		#
		#大盘函数
		return $this->index($this->today,$aMarket,'count');
	}
	final 
	function INDEXA($aMarket="sh"){
		#1， INDEXA 　 返回大盘成交额，如：D:INDEXA,输出大盘成交额;
		return $this->index($this->today,$aMarket,'amount');
	}
	final 
	function INDEXADV($aMarket="sh"){
		#2， INDEXADV 返回上涨家数
		return $this->index($this->today,$aMarket,'up');
	}
	final 
	function INDEXDEC($aMarket="sh"){
		#3， INDEXDEC 返回下跌家数
		return $this->index($this->today,$aMarket,'down');
	}
	final 
	function INDEXC($aMarket="sh"){
		#4， INDEXC 　 返回大盘收盘价
		return $this->index($this->today,$aMarket,'close');
	}
	final 
	function INDEXH($aMarket="sh"){
		#5， INDEXH 　 返回大盘最高价
		return $this->index($this->today,$aMarket,'high');
	}
	final 
	function INDEXL($aMarket="sh"){
		#6， INDEXL 　 返回大盘最低价
		return $this->index($this->today,$aMarket,'low');
	}
	final 
	function INDEXO($aMarket="sh"){
		#7， INDEXO 　 返回大盘开盘价
		return $this->index($this->today,$aMarket,'open');
	}
	final 
	function INDEXV($aMarket="sh"){
		#8， INDEXV 　 返回大盘成交量
		return $this->index($this->today,$aMarket,'volume');
	}
};
